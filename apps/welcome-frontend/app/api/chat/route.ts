...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // Timeout di sicurezza sincronizzato con l'agent (100 minuti)
        ...
        } = await undiciRequest(`${backendUrl}/api/v1/chat`, {
            method: 'POST',
            headers: {
        ...
        if (error.name === 'AbortError') {
            return new Response(
                JSON.stringify({
                    error: 'Il server ha impiegato troppo tempo a rispondere (Timeout)',
                }),
                { status: 504 }
            )
        }
