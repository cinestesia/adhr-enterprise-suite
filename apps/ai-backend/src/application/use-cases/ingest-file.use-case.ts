...
export class IngestFileUseCase {
    ...
    async createDataWithMetadata(fileBuffer: Buffer, fileName: string, department: string) {
    ...
    async execute(dto: IngestRequestDTO) {
        ...
        try {
            ...
            const dataWithMetadata = await this.createDataWithMetadata(fileBuffer, fileName, department)
