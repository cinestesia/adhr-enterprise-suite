...
export class SessionService {
    ...
    */
    ...
    async resolve(sessionId: string | undefined, userId: string, department: string): Promise<{ session: ChatSession; isNew: boolean }> {
        ...
        if (!isOwner) {
            throw new Error('Accesso negato: sessione non valida o non appartenente all\'utente');
        }
     ...
     */
    async generateTitleInBackground(sessionId: string, userId: string, firstMessage: string): Promise<void> {
        try {
