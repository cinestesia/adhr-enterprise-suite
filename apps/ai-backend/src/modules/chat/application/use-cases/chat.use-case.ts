    ...
    async execute(dto: ChatRequestDTO): Promise<ChatUseCaseOutput> {
        ...
        try {
            ...
            const { session, isNew } = await this.sessionService.resolve(
                providedId,
                user.id,
                user.mainDepartment // Attualmente utilizzato solo per generare un titolo provvisiorio
            )
            ...
            // Inizio dell'analisi del mesaggio corrente per capire quale rotta prendere (RAG, AGENT, GENERIC)
            const analysis = await this.queryAnalyzer.analyze(message)

            /**
             * @note
             * analyze utilizza un llm come ad esempio ollama, come "cervello" per fare la pulizia
