...
export const BaseChatRequestSchema = z.object({
    message: z.string().min(1, "Il messaggio è obbligatorio").describe("Il messaggio inviato dall'utente"),
...
export const ChatRequestSchema = BaseChatRequestSchema.extend({
    user: z.instanceof(User, { message: "L'utente deve essere un'istanza valida del dominio" })
})
